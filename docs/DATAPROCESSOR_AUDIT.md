...
- **Model signature (`models_used[]`)**: **FAIL** (в обоих NPZ `models_used` отсутствует).
...
Основание: `docs/project_questions.md` (раздел 6.1) + цель проекта “привести процессоры к одной структуре”.
...
| `analysis_fps` | Segmenter + Root orchestrator (дефолты/передача) | `DataProcessor/main.py` (передача/дефолт), `Segmenter/segmenter.py` (писать) | **FAIL** |
| `analysis_width/analysis_height` | Segmenter + Root orchestrator (дефолты/передача) | `DataProcessor/main.py` (передача/дефолт), `Segmenter/segmenter.py` (писать) | **FAIL** |
...
- **`analysis_*`**: отсутствуют в frames metadata и не задаются дефолты → ломает контракт `SEGMENTER_CONTRACT.md`.
- **`models_used[]`/`model_signature`**: отсутствуют в NPZ → ломает idempotency key по `MODEL_SYSTEM_RULES.md`.
...
- **Критерий**: `analysis_fps`, `analysis_width/height`, `color_space="RGB"` фиксированы.
...
- `Segmenter/segmenter.py`:
  ...
  - `process_video_union()`: сохраняет RGB кадры в `batch_*.npy`, пишет `union_frame_indices_source` + `union_timestamps_sec`, `height/width/channels`.
...
- **analysis timeline fields (`analysis_fps`, `analysis_width`, `analysis_height`)**: **FAIL**
  - `SEGMENTER_CONTRACT.md` требует явных `analysis_*` полей и дефолтов (например `analysis_fps=12`, `analysis_height=320`).
  - `Segmenter/segmenter.py` сейчас пишет `fps=source_fps` и `height/width` как фактический output размер, но не пишет `analysis_fps` и не пробрасывает `analysis_*` по умолчанию.
  - Оркестратор `main.py` не передаёт `--analysis-width/--analysis-height` в Segmenter.
- **`dataprocessor_version` в metadata**: **FAIL**
  - поле отсутствует, Segmenter не получает/не пишет его.
...
- **No-fallback policy (dependencies)**: **PARTIAL**
  ...
  - `EmotionFaceModule`/`VideoEmotionProcessor.frames_with_face_load()` содержит fallback чтения результатов `face_detection` напрямую из файлов/legacy путей (нарушает “no-fallback/единый источник”).
...
PASS/FAIL (первичный, по коду):
- **Per-run NPZ artifact + manifest merge**: **PASS** (паттерн как у AudioProcessor).
- **`dataprocessor_version`**: **FAIL**
  - не пишется ни в `manifest.run`, ни в NPZ meta (аналогично AudioProcessor).
- **Model signature (`models_used[]`)**: **FAIL**
  - `model_registry.get_model()` создаёт SentenceTransformer, но `run_cli.py` не фиксирует `models_used[]` в meta.
- **Privacy (raw text)**: **FAIL/Gap (высокий риск)**
  - `run_cli.py` сохраняет `payload` целиком в NPZ (`payload=np.asarray(payload, dtype=object)`).
  - без явной политики/флагов это может сохранять raw title/description/transcripts/comments, что конфликтует с `PRIVACY_AND_RETENTION.md`.
  - Требуется: либо санитизация payload, либо раздельный “debug payload” под флагом и хранение только агрегатов по умолчанию.
- **Embeddings cache**: **PARTIAL**
  - есть registry моделей (`model_registry.py`), но не видно стандартизированного disk cache с ключом `hash(text + model_signature + preprocessing_flags)`.
  - при появлении disk cache обязателен ключ с `model_signature` (см. `MODEL_SYSTEM_RULES.md`).
...
- **`models_used[]` / `model_signature`**: **FAIL**
  - Отсутствует во всех проверенных NPZ, включая core providers (`core_clip`, `core_depth_midas`).
...
- **DAG (MVP)**: `docs/component_graph.yaml` как source-of-truth; baseline/v1/v2 имеют отдельные DAG.
