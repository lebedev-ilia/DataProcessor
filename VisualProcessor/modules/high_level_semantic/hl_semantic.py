...
Inputs (examples):
- scene_frames: list[list[np.uint8]] or list of representative PIL/np frames per scene
- OR scene_embeddings: np.array [n_scenes, D] (if precomputed)
- scene_metadata: list of dicts with scene_id, start_ts, end_ts, representative_frame_idx, scene_duration, scene_source_confidence
...
- mode: "fast" or "full" (default: "full")
...
- per_scene_vectors: np.array [n_scenes, 64] for VisualTransformer
"""
...
# Optional: DTW for emotion alignment
try:
...
class HighLevelSemanticsOptimized:
    ...
    def _load_clip_model(self, model_name):
        if _CLIP_BACKEND == "openai":
            ...
            # note: openai clip returns 512- or 768-d embeddings depending on model
        else:
            # open_clip
            import open_clip
    ...
    def zero_shot_genre_style(
        ...
        """
        if _CLIP_BACKEND is None:
            return {"genre_probabilities": {}, "per_scene_top_class": [], "dominant_genre": None, "genre_confidence": 0.0}
        ...
        # Project scene embeddings if needed (they should already be projected)
        # But text embeddings are raw CLIP, so we need to project them too for fair comparison
        # Actually, for zero-shot, we compare raw CLIP embeddings
        # So we need to get raw scene embeddings - but we only have projected ones
        # For now, compare projected scene embeddings with projected text embeddings
    ...
    # -------------------------
    # Scene semantic features (improved naming)
    # -------------------------
...
if __name__ == "__main__":
    ...
    print("Per-scene vectors shape:", out["per_scene_vectors"].shape)
