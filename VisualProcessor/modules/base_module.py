...
class BaseModule(ABC):
    ...
    def load_core_provider(
        ...
        # Core провайдеры сохраняют файлы в корне rs_path
        if file_name:
            file_path = os.path.join(self.rs_path, file_name)
            ...
            # Автоматический поиск: пробуем стандартные имена
            common_names = [
            ...
            for name in common_names:
                file_path = os.path.join(self.rs_path, name)
                if os.path.exists(file_path):
                    return self._load_npz(file_path)
            
            # Если не нашли стандартные, ищем любые npz в корне
    ...
    def save_results(
        ...
        core_dir = os.path.join(self.rs_path, self.module_name)
        os.makedirs(core_dir, exist_ok=True)
        npz_path = os.path.join(core_dir, f"{self.module_name}_features.npz")
    ...
    def run(
        ...
        frame_indices = self.get_frame_indices(metadata)
