...
def main():
    ...
    # Build frame sampling indices
    frame_indices = list(range(0, total_frames, 10))
    LOGGER.warning(f"{NAME} | main | sampled frames count: {len(frame_indices)}"
        "(Это фиксированая выборка для провайдера depth_midas, но использовать ее могут разные модули с разной "
    ...
    # Prepare output dirs
    core_dir = os.path.join(args.rs_path, "core_depth_midas")
    ...
    out_path = os.path.join(args.rs_path, "depth.npz")
