...
def main():
    ...
    parser.add_argument("--tracker-fps", type=int, default=30)
    ...
    frame_indices = list(range(0, total_frames, args.sample_step))
    LOGGER.warning(
        "%s | sampled frames: %d / %d (step=%d)",
        NAME,
        len(frame_indices),
        total_frames,
        args.sample_step,
    ...
    try:
        ...
        try:
            tracker_cfg = {"track_thresh": 0.25, "match_thresh": 0.8, "frame_rate": args.tracker_fps}
        ...
        # prepare class_names array "id:name"
        class_names_arr = np.array([f"{k}:{v}" for k, v in sorted(class_names.items())], dtype="U")

        # prepare tracks_list as object array (one entry per track id sorted)
        if tracks_map:
            ...
            # also save mapping track_ids -> index in tracks_list
